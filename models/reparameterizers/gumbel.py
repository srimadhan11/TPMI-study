...
class GumbelSoftmax(nn.Module):
    ...
    def _soft_prior(self, batch_size):
        unif = uniform([batch_size, self.output_size], cuda=self.config['cuda'])
    ...
    def anneal(self, anneal_interval=10):
        ...
        if self.training and self.iteration > 0 and self.iteration % anneal_interval == 0:
            # smoother annealing
    ...
    def reparmeterize(self, logits):
        ...
        z, z_hard = self.sample_gumbel(logits, self.tau, hard=True, use_cuda=self.config['cuda'])
    ...
    def kl(self, dist_a):
        return torch.sum(GumbelSoftmax._kld_categorical_uniform(dist_a['discrete']['log_q_z']), dim=-1)
    ...
    def sample_gumbel(x, tau, hard=False, use_cuda=True):
        ...
        if hard:
            ...
            y_hard      = Variable(torch.eq(y_max.data, y.data).type(float_type(use_cuda)))
    ...
    def forward(self, logits):
        ...
        self.iteration += 1
        return z if self.training else z_hard, { 'z': z, 'discrete': params }
